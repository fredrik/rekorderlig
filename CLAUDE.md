...
- Everything is synchronous around SQLite. Voting only records the vote; the
  ...
  whole corpus (~50k stories, about 0.8 s — batched, see `RESCORE_BATCH`). **A round boundary is the only retrain trigger** — no
...
- **Peak memory is a design constraint, not an afterthought.** The machine is
  `shared-cpu-1x` with **256 MB** (`fly.toml`), and two Node processes routinely
  share it: the server, and a `cli.js sync`/`backfill` run over `fly ssh`. Both
  of the big loops used to load their whole working set at once — a rescore held
  every story (182 MB peak), a backfill held every fetched item for a day
  (~164 MB) — and finishing a training round while a backfill was running was
  enough to get the server OOM-killed and the box rebooted, taking the backfill
  with it. Both now stream: peak is flat in the size of the corpus and in the
  length of a day's id range. Keep new bulk loops the same shape — a `.all()`
  over `stories` is the shape to avoid — and note that the JS heap is *not* the
  thing to watch: it never exceeded 6 MB in any of this, while RSS ran to 182 MB.
  Measure with `/usr/bin/time -l node src/cli.js …`.
