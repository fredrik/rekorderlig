...
/// The invites table as migration 3 created it, frozen: a migration is never
/// edited, and this is the shape migration 5 starts from (migration 4, the
/// learning-curve columns, does not touch invites). The fresh path uses
/// `INVITES_TABLE` below.
const INVITES_TABLE_V3: &str = "
...
/// Invites, shared verbatim by the fresh path and (in pieces) migration 5.
///
...
///
/// Two references to `users`, and they answer different questions.
/// `invited_by` is who **sent** it — known when the row is written, because
/// any user may invite a friend from Brain — and NULL when the operator
/// minted it, which is the honest answer: the operator is not a user and has
/// no row to point at. `user_id` is who it **became**, and is NULL until
/// someone opens it. Both are `ON DELETE SET NULL`, so removing a user
/// leaves the ledger's shape and loses only the name.
const INVITES_TABLE: &str = "
CREATE TABLE IF NOT EXISTS invites (
  id          BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
  token_hash  BYTEA  NOT NULL UNIQUE,
  note        TEXT,                    -- who the sender meant it for
  created_at  BIGINT NOT NULL,
  expires_at  BIGINT NOT NULL,
  redeemed_at BIGINT,                  -- NULL until someone opens it
  ...
  -- erase the fact that an invite was taken up, only who is left of it.
  user_id     BIGINT REFERENCES users(id) ON DELETE SET NULL,
  -- Who sent it; NULL when the operator did. Last, where ADD COLUMN put it.
  invited_by  BIGINT REFERENCES users(id) ON DELETE SET NULL
);
CREATE INDEX IF NOT EXISTS idx_invites_created ON invites(created_at DESC);
-- A user's own list, newest first: the ledger they are shown in Brain.
...
/// 4 → 5: an invite records who sent it. `docs/multi-user.md`, "Invites".
///
/// Nullable with no default and no backfill, deliberately: every invite that
/// exists when this runs was minted by the operator, and NULL is exactly what
/// "the operator sent it" is spelled as from now on. Stamping them with user 1
/// would invent a fact — user 1 is the owner, not the operator.
const MIGRATION_4: &str = "
ALTER TABLE invites ADD COLUMN invited_by BIGINT REFERENCES users(id) ON DELETE SET NULL;
CREATE INDEX idx_invites_invited_by ON invites(invited_by, created_at DESC);
";
...
pub struct InviteRecord {
    ...
    /// Who sent it — `None` when the operator did, and again if that user has
    /// since been removed.
    #[serde(rename = "invitedBy")]
    pub invited_by: Option<UserRecord>,
}

/// Mint an invite. `note` is for the sender's own list and reaches nobody
/// else. `invited_by` is who is sending it, and `None` means the operator:
/// they have no user row to point at, and the ledger says so rather than
/// guessing at one.
///
...
/// read.
pub fn create_invite(db: &Db, note: Option<&str>, invited_by: Option<User>) -> Invite {
    let token = mint_token(db);
    let now = now_seconds();
    let expires_at = now + LINK_TTL_SECS;
    ...
    let id: i64 = db
        .query_one(
            &format!(
                "INSERT INTO invites (token_hash, note, created_at, expires_at, invited_by)
                 VALUES ({TOKEN_HASH}, $2, $3, $4, $5) RETURNING id"
            ),
            &[&token, &note, &now, &expires_at, &invited_by],
        )
...
/// The ledger's columns and its two joins: `u` is who the invite became, `b`
/// is who sent it. Both `LEFT`, and for different reasons — an unredeemed
/// invite has no user, and an invite the operator sent has no sender.
...
SELECT i.id, i.note, i.created_at, i.expires_at, i.redeemed_at, i.revoked_at,
       ...
       b.id, b.display_name, b.email, b.created_at
  FROM invites i
  LEFT JOIN users u ON u.id = i.user_id
  LEFT JOIN users b ON b.id = i.invited_by";

/// One ledger row, from `INVITE_SELECT`'s columns.
fn invite_from_row(r: &Row) -> InviteRecord {
    InviteRecord {
        id: r.get(0),
        note: r.get(1),
        created_at: r.get(2),
        ...
        invited_by: r.get::<_, Option<User>>(10).map(|id| UserRecord {
            id,
...
/// The ledger, newest first: every invite ever minted, whoever sent it, and
/// what became of it. The operator's view — a user gets `list_invites_by`.
pub fn list_invites(db: &Db) -> Vec<InviteRecord> {
    db.query(
        &format!("{INVITE_SELECT} ORDER BY i.created_at DESC, i.id DESC"),
        &[],
    )
    .expect("list_invites")
    .iter()
...
/// One user's own invites, newest first: the ones they sent, and nobody
/// else's. The `invited_by` predicate is the whole isolation rule here — with
/// it left off, one friend's list is everybody's.
pub fn list_invites_by(db: &Db, user: User) -> Vec<InviteRecord> {
    db.query(
        &format!(
            "{INVITE_SELECT}
    ...
    .expect("list_invites_by")
    .iter()
    .map(invite_from_row)
    .collect()
}

/// How many of a user's invites are still live: unopened, unvoided, unexpired.
/// The one number a cap on inviting can be about — the ones already taken up
/// are people, not outstanding links.
pub fn outstanding_invites(db: &Db, user: User) -> i64 {
    db.query_one(
...
///
/// `by` is who is asking: `Some(user)` may only void what they sent
/// themselves, and `None` is the operator, who may void any row. One
/// statement for both, because the difference between them is one predicate
/// and splitting it into two functions is how the predicate gets forgotten.
pub fn revoke_invite(db: &Db, id: i64, by: Option<User>) -> bool {
    let mut sql = "UPDATE invites SET revoked_at = $2
         WHERE id = $1 AND redeemed_at IS NULL AND revoked_at IS NULL"
        .to_string();
    let now = now_seconds();
    let mut params: Vec<&(dyn ToSql + Sync)> = vec![&id, &now];
    if let Some(user) = by.as_ref() {
        sql.push_str(" AND invited_by = $3");
        params.push(user);
    }
    db.execute(&sql, &params).expect("revoke_invite") > 0
}
