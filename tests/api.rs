...
fn start(name: &str, auth_token: Option<&str>) -> TestServer {
    ...
    let app = App::new(db.path.clone(), public_dir(), auth_token.map(str::to_string));
    let handle = serve(Arc::clone(&app), "127.0.0.1:0");
    let base = format!("http://127.0.0.1:{}", handle.port);
    TestServer { app, handle, base, _db: db }
}
...
fn seed_api_stories(app: &App) {
    ...
    for (id, title, url) in rows {
        ...
        upsert_story(
            &conn,
            &story(id, title, Some(url), Some(&host), &format!("u{id}"), 100 - id, 100 - id, now - id * 60),
        );
    }
}
...
fn malformed_requests_are_4xx_not_500() {
    ...
    assert_eq!(post(base, "/api/vote", json!({"id": 9999, "value": 1})).0, 404);
    ...
    let no_id = post(base, "/api/import/vote", json!({"value": 1, "created_at": 1787574980}));
    assert_eq!(no_id.0, 400);
    assert!(no_id.1["error"].as_str().unwrap().contains("story_id"));
    assert_eq!(
        post(base, "/api/import/vote", json!({"story_id": 1, "value": 7, "created_at": 1787574980})).0,
        400
    );
...
fn the_api_flow_votes_train_rerank_export_import() {
    ...
    for id in [1, 2, 3] {
        assert_eq!(post(base, "/api/vote", json!({"id": id, "value": 1})).0, 200);
    }
    for id in [4, 5] {
        assert_eq!(post(base, "/api/vote", json!({"id": id, "value": -1})).0, 200);
    }
    ...
    assert!(last.1.get("training").is_none(), "voting only records; training is a separate trigger");
    assert_eq!(last.1["votes"], json!({"up": 3, "down": 3, "skip": 0, "total": 6}));
    ...
    assert_eq!(feed["items"][0]["id"], 7, "the unseen Rust story should rank first");
    ...
    let ids: Vec<i64> = explore["items"].as_array().unwrap().iter().map(|s| s["id"].as_i64().unwrap()).collect();
    assert_eq!(ids, vec![7]);
    let card = &explore["items"][0];
    let expected_tier = if card["score"].as_f64().unwrap() >= 0.6 { "probably" } else { "possibly" };
    assert_eq!(card["tier"], expected_tier);
    assert!(
        card["points"].as_i64().unwrap() >= explore["bar"]["minPoints"].as_i64().unwrap()
            || card["num_comments"].as_i64().unwrap() >= explore["bar"]["minComments"].as_i64().unwrap()
    );
    ...
    // The traction bar is the whole point: a story nobody engaged with stays out.
    {
        ...
        upsert_story(
            &conn,
            &story(42, "Rust ownership, one more time", Some("https://rustblog.dev/quiet"), Some("rustblog.dev"), "u42", 2, 1, now - 120),
        );
    }
    ...
    {
        let conn = server.app.db.lock().unwrap();
        conn.execute("DELETE FROM stories WHERE id = 42", []).unwrap();
    }
    ...
    assert!(strict["items"].as_array().unwrap().iter().all(|s| s["score"].as_f64().unwrap() >= 0.55));
    ...
    assert_eq!(stats["model"]["rev"], before, "the client decides when to retrain");
    ...
    assert_eq!(back.1["fetched"], false, "story 1 is already in the corpus, so no HN lookup");
    assert_eq!(back.1["story"]["title"], one["title"], "the stored story comes back for eyeballing");
    assert_eq!(get(base, "/api/stats").1["votes"]["total"], 5);
    {
        let conn = server.app.db.lock().unwrap();
        let created: i64 = conn
            .query_row("SELECT created_at FROM votes WHERE story_id = 1", [], |r| r.get(0))
            .unwrap();
        assert_eq!(created, one["created_at"].as_i64().unwrap(), "the historical vote time is kept, not stamped with now");
    }
    ...
    assert_eq!(get(base, "/api/stats").1["votes"]["total"], 5);
    {
        ...
        let (created, updated): (i64, i64) = conn
            .query_row("SELECT created_at, updated_at FROM votes WHERE story_id = 1", [], |r| {
                Ok((r.get(0)?, r.get(1)?))
            })
            .unwrap();
        assert_eq!(created, then);
        assert_eq!(updated, then, "a restored vote is the row as it was, not touched-just-now");
    }
    ...
    let restored = listed["items"].as_array().unwrap().iter().find(|v| v["id"] == 1).unwrap();
    ...
    assert_eq!(all_votes["counts"], json!({"up": 3, "down": 2, "skip": 0, "total": 5}));
    let listed_ids: Vec<i64> =
        all_votes["items"].as_array().unwrap().iter().map(|r| r["id"].as_i64().unwrap()).collect();
    ...
    assert!(up["items"].as_array().unwrap().iter().all(|r| r["vote"] == 1));

    let page = get(base, "/api/votes?limit=2&offset=2").1;
    assert_eq!(page["total"], 5, "total counts every match, not just the page");
    let page_ids: Vec<i64> =
        page["items"].as_array().unwrap().iter().map(|r| r["id"].as_i64().unwrap()).collect();
    ...
    assert!(days_body["older"].is_null(), "nothing in this corpus predates the window");
    ...
    assert!(days.iter().all(|d| day_re.is_match(d["day"].as_str().unwrap())));
