#!/usr/bin/env bash
# Set the three secrets that point everything at the database: DATABASE_URL on
# the app, and the two preview passwords the PR workflow builds its own URLs
# from. Run it after the roles exist (scripts/fly-db-setup.sh prints the SQL),
# and again whenever a password is rotated.
#
